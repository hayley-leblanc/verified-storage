...
#[cfg(windows)]
use crate::pmem::pmemfile_t::{FileBackedPersistentMemoryRegions, MemoryMappedFileMediaType};
...
verus! {
    ...
    //     if let Ok(mut pm_regions) =
    //         FileBackedPersistentMemoryRegions::new(&dir_name, MemoryMappedFileMediaType::SSD, region_sizes.as_slice()) {
    ...
    //             Err(_) => return false, // if setup fails, end the test
    ...
    //                         Ok(pos) => runtime_assert(pos == 0),
    //                         _ => return false // if the append fails, end the test
    //                     }
